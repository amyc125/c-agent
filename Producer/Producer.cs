        ...
        public async Task StartSendingMessages(string topicName)
        {
            ...
            try
            {
                ...
                for(int i=myCount-1;i>-1;i--)
                {
                    ...
                    var sendEvents = await producer.ProduceAsync(topicName,
                    ...
                    Console.WriteLine($"Message sent (value: '{event1}'). Delivery status: {sendEvents.Status}");
                    
                    if (sendEvents.Status != PersistenceStatus.Persisted)
                    {
                        // delivery might have failed after retries. This message requires manual processing.
                        Console.WriteLine(
                            $"ERROR: Message not ack'd by all brokers (value: '{event1}'). Delivery status: {sendEvents.Status}");
                    }
                }
            }
